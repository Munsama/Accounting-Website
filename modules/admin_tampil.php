...
<div class="container-table">
	...
	<input type="button" class="btn-biru" value="Tambah Data" style="float:left;" onclick="window.location.href='admin_tambah.php'">
	...
	<?php
		...
		$query	= mysql_query("SELECT * FROM _admin where usernames LIKE '%$keyword%'")or die (mysql_error());
			while ($hasil = mysql_fetch_array($query)) {
				...
				echo"<td width='25%'>
					<a href='admin_tambah.php&usernames=$hasil[usernames]' class='btn-biru'>Edit</a>
...
for($i=1; $i<=$jml_data; $i++){
	if($i!=$batas){
		echo "<a href='admin.php&hal=$i'><button style='padding:5px; background-color:#fff;'>$i</button></a>";
	}
		else {
